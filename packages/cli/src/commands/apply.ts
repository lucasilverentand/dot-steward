...
async function applyWithListr(
  ...
  for (const r of rows) {
    const it = mgr.deps.nodes.get(r.id) as unknown as
      | { kind?: string }
      | undefined;
    if (it && it.kind === "plugin") pluginRows.push(r);
  }
  if (pluginRows.length > 0)
    sections.push({ title: "Plugins", rows: pluginRows });
  ...
  const offErr = mgr.events.on("item:apply_error", (ev) => {
    ...
    if (row) {
      ...
      if (isTTY) draw();
        ...
        logger.error(`${pc.red("✖")} ${row.title} ${pc.dim("->")} ${ev.error}`);
    }
  });
  const offSkip = mgr.events.on("item:apply_skip", (ev) => {
    ...
    if (row) {
      ...
      if (isTTY) draw();
        ...
        logger.log(
          `${pc.yellow("↷")} ${row.title} ${pc.dim("->")} ${row.note}`,
        );
    }
  });
  ...
  await Promise.allSettled([
    applyPromise,
    ...[...waiters.values()].map((w) => w.promise),
  ]);
  stopSpinner();
  ...
  try {
    const counts: SummaryCounts = {
      plus:
        summaryCounts?.plus ?? rows.filter((r) => r.status === "done").length,
      minus: summaryCounts?.minus ?? 0,
      tilde:
        summaryCounts?.tilde ?? rows.filter((r) => r.status === "skip").length,
  ...
  program
    ...
    .action(
      async (opts: {
        ...
        const useSavedPlan = false;
        type SavedDecision = import("../state.ts").SavedDecision;
        const last: { decisions: SavedDecision[] } | undefined = undefined;
        ...
        if (process.env.DS_USE_SAVED_PLAN === "1") {
          ...
          const reconstructed: PlanDecision[] = await reconstructSavedDecisions(
            mgr,
            (last?.decisions ?? []) as SavedDecision[],
          );
          ...
          const toApplySaved = reconstructed.filter(
            (d) => d.action === "apply",
          ).length;
          if (toApplySaved === 0 && removedSavedCount === 0) {
            logger.log("│");
            logger.log(
              `╰─  ${pc.dim("Nothing to do. All items are up-to-date.")}`,
            );
          ...
          // Confirm apply using the saved plan preview above
          {
            ...
            const toApplySavedCount = reconstructed.filter(
              (d) => d.action === "apply",
            ).length;
            const promptMsg =
            ...
            if (!proceed) {
              ...
              result = {
                ...
                stats: {
                  ...
                  to_apply: reconstructed.filter((d) => d.action === "apply")
                    .length,
                  skipped: reconstructed.filter((d) => d.action === "skip")
                    .length,
              ...
              const plannedList = reconstructed
                .filter((d) =>
                  opts.force ? d.action !== "skip" : d.action === "apply",
                )
                .map((d) => ({
                  id: d.item.id,
              ...
              try {
                if (plannedList.length > 0) {
                  ...
                  const minus = reconstructed.filter(
                    (d) => d.action === "noop",
                  ).length;
                  const tilde = reconstructed.filter(
                    (d) => d.action === "skip",
                  ).length;
                ...
                result = {
                  ...
                  stats: {
                    ...
                    skipped: reconstructed.filter((d) => d.action === "skip")
                      .length,
                    noop: reconstructed.filter((d) => d.action === "noop")
                      .length,
                  },
                  errors: [],
                };
          ...
          const toApplyCount = decisions.filter(
            (d) => d.action === "apply",
          ).length;
            ...
            if (!proceed) {
              ...
              result = {
                ...
                stats: {
                  ...
                  to_apply: decisions.filter((d) => d.action === "apply")
                    .length,
