...
function getTTYWidth(): number {
  const w = (process.stdout && (process.stdout as unknown as { columns?: number; isTTY?: boolean }).columns) || 0;
  return w && w > 0 ? w : 80;
}
...
export function renderTable(
  ...
  let widths = Array.from({ length: cols }, (_, i) => {
    ...
    const colLen = Math.max(0, ...data.map((r) => (r[i] ? stripAnsi(r[i]).length : 0)));
  ...
  if (current > available) {
    ...
    widths = widths.map((w) => Math.max(minCol, Math.floor(minCol + (w - minCol) * scale)));
  }
...
export function renderKeyValueBox(
  ...
  if (opts?.labelMax && opts.labelMax > 0) labelWidth = Math.min(labelWidth, opts.labelMax);
  if (opts?.valueMax && opts.valueMax > 0) valueWidth = Math.min(valueWidth, opts.valueMax);
  ...
  if (insideWidth > maxInside) {
    ...
    const labelAlloc = Math.max(minLabel, Math.floor((labelWidth / Math.max(1, total)) * alloc));
...
export function renderListBox(
  ...
  const contentWidth = Math.max(0, ...rows.map((r) => stripAnsi(r).length), opts?.title ? stripAnsi(opts.title).length : 0);
...
export function renderKeyValueGridBox(
  ...
  const data = pairs.map(([k, v]) => [toCell(k), toCell(v)] as [string, string]);
  ...
  if (opts?.labelMax && opts.labelMax > 0) {
    for (let c = 0; c < cols; c++) labelW[c] = Math.min(labelW[c], opts.labelMax);
  }
  if (opts?.valueMax && opts.valueMax > 0) {
    for (let c = 0; c < cols; c++) valueW[c] = Math.min(valueW[c], opts.valueMax);
  }
  ...
  if (sumCol > available) {
    ...
    for (let i = 0; i < cols; i++) {
      ...
      const labAlloc = Math.max(3, Math.floor((labelW[i] / totalOrig) * content));
