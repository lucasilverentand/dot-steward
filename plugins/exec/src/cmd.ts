...
export class ExecCommand extends Item {
  ...
  constructor(
    ...
    readonly options?: ExecCommandOptions,
    plugin?: ExecPlugin,
  ) {
  ...
  async apply(ctx: HostContext): Promise<void> {
    ...
    if (!cwd)
      throw new Error(
        "exec: host home directory is unknown; set options.cwd or ensure HostContext.user.home",
      );
    ...
    if (!res.ok)
      throw new Error(`exec apply failed: ${res.stderr || "command error"}`);
  }
  ...
  async cleanup(ctx: HostContext): Promise<void> {
    ...
    if (!res.ok)
      throw new Error(`exec cleanup failed: ${res.stderr || "command error"}`);
  }
}
