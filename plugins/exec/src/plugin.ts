...
export class ExecPlugin extends Plugin {
  ...
  async run(
    cmd: string,
    opts?: { shell?: ShellKind; cwd?: string; env?: Record<string, string>; sudo?: boolean | "auto"; sudoUser?: string },
    ctx?: HostContext,
    ...
    let res = await runShell(cmd, { shell: opts?.shell, cwd: opts?.cwd, env: opts?.env, sudo: useSudo, sudoUser: opts?.sudoUser });
    if (useSudo && !res.ok && ctx && ctx.os !== "win32" && !ctx.user.is_root && ctx.user.can_sudo) {
      try {
        const r = spawnSync("sudo", ["-v"], { stdio: "inherit" });
        if (r.status === 0) {
          res = await runShell(cmd, { shell: opts?.shell, cwd: opts?.cwd, env: opts?.env, sudo: true, sudoUser: opts?.sudoUser });
        }
      } catch {}
    }
    return res;
